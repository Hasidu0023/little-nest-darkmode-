...
class UpdateTeacherFragment :
    ...
    private fun fetchTeacher() {
        ...
        if (profileId == -1) {
            Toast.makeText(requireContext(), "Unable to load profile. Please log in again.", Toast.LENGTH_SHORT).show()
            handleLogout()
            return
        }
    ...
    private fun setupObservers() {
        ...
        teacherViewModel.teacherById.observe(viewLifecycleOwner) { result ->
            if (result == null) {
