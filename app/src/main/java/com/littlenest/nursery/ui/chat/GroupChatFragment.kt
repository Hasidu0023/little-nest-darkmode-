...
class GroupChatFragment : BaseFragment(R.layout.fragment_chat) {
...
//        val chat = args.chat
//        val groupId = chat.groupId ?: return
//        val groupName = chat.groupName
//        val groupImage = chat.partnerProfilePicture // optional icon
        ...
        // Inflate a custom layout inside toolbar
        ...
        nameView.text = groupName ?: "Group Chat"

        if (!groupImage.isNullOrEmpty()) {
            Glide.with(requireContext())
                .load(getBaseUrl() + groupImage)
                .placeholder(R.drawable.ic_group_chat)
                .circleCrop()
                .into(avatarView)
        } else {
            avatarView.setImageResource(R.drawable.ic_group_chat)
        }
    ...
    private fun sendGroupMessage(groupId: Int, messageText: String?) {
